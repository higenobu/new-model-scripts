...
from utils import EconIndicatorDataset
from data_utils import process_data_pipeline
...
def main(args):
	...
	sentences_train_combined, labels_train_combined, sentences_dev_combined, \
		labels_dev_combined, sentences_test_combined, labels_test_combined = process_data_pipeline(args.data_path)
	...
	for i in range(1, 8):
		df_list_trains.append(pd.DataFrame({"sentence": sentences_train_combined[i], "label": labels_train_combined[i]}))
		df_list_devs.append(pd.DataFrame({"sentence": sentences_dev_combined[i], "label": labels_dev_combined[i]}))
		df_list_tests.append(pd.DataFrame({"sentence": sentences_test_combined[i], "label": labels_test_combined[i]}))
